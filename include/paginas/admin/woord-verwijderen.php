...
{
	$query = mysqli_query($con, "SELECT game_id FROM raad_het_woord WHERE woord_id = '".$game_id."'");
	if(mysqli_num_rows($query) == 1)
	{
		$row = mysqli_fetch_array($query);
		$gameId = $row['game_id'];
	}
	$query = mysqli_query($con, "DELETE FROM raad_het_woord WHERE woord_id = '".$game_id."'");
	
	$query = mysqli_query($con, "SELECT game_id FROM raad_het_woord WHERE game_id = '".$gameId."'");
