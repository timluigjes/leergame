...
{	
	$query = mysqli_query($con, "SELECT game_id FROM quiz WHERE quiz_id = '".$game_id."'");
	if(mysqli_num_rows($query) == 1)
	{
		$row = mysqli_fetch_array($query);
		$gameId = $row['game_id'];
	}
	$query = mysqli_query($con, "DELETE FROM quiz WHERE quiz_id = '".$game_id."'");
	
	$query = mysqli_query($con, "SELECT game_id FROM quiz WHERE game_id = '".$gameId."'");
...
<h1>Vraag verwijderen</h1>
<div id="instellingencontent">
<p style="text-align:center;">Weet u zeker dat u deze vraag wilt verwijderen?</p>
